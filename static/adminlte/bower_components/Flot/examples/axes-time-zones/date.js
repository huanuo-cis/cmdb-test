...
//
// The other significant difference from the built-in JavaScript Date is that `timezoneJS.Date` also has named properties that store the values of year, month, data, etc., so it can be directly serialized to JSON and used for data transfer.
...
(function () {
  ...
  // `_fixWidth(1998, 2) = '98'`
  //
  // This is used to pad numbers in converting data to string in ISO standard.
  ...
  timezoneJS.Date = function () {
    ...
    //
    // - `year, month, [data,] [hours,] [minutes,] [seconds,] [millis,] [tz]: Same as `Date` object
    // with tz.
    //
  ...
  timezoneJS.timezone = new function () {
    ...
    //if isUTC is true, data is given in UTC, otherwise it's given
    // in local time (ie. data.getUTC*() returns local time components)
    function getRule(dt, zone, isUTC) {
      var date = typeof dt === 'number' ? new Date(dt) : dt;
      var ruleset = zone[1];
      var basicOffset = zone[0];

      //Convert a data to UTC. Depending on the 'type' parameter, the data
      // parameter may be:
      //
    ...
    // - `w`: wall clock time (adjust for both time zone and DST offset).
      ...
      //
      //Step 2:  Sort the rules by effective data.
      //
      //Step 3:  Check requested data to see if a rule has yet taken effect this year.  If not,
      //
      ...
      //
      //Step 5:  Sort the rules by effective data.
      //Step 6:  Apply the most recent rule before the current time.
      var convertRuleToExactDateAndTime = function (yearAndRule, prevRule) {
        var year = yearAndRule[0]
          , rule = yearAndRule[1];
          // Assume that the rule applies to the year of the given data.
        ...
        if (EXACT_DATE_TIME[year][rule])
          ...
          //If we have a specific data, use that!
          if (!isNaN(rule[4])) {
            effectiveDate = new Date(Date.UTC(year, SHORT_MONTHS[rule[3]], rule[4], hms[1], hms[2], hms[3], 0));
          }
          //Let's hunt for the data.
          else {
            var targetDay
              , operator;
            //Example: `lastThu`
      ...
      // As the given data may indeed be close to a DST change, it may get sorted
