...
test('update - null', function(){
    ...
    equal(selected.length, 0, 'No data is selected');
    strictEqual(returnedObject, this.dp, "is chainable");
});
...
test('moveMonth - can handle invalid data', function(){
    // any input which results in an invalid data, f.e. an incorrectly formatted.
    var invalidDate = new Date("invalid"),
        returnedObject = this.dp.moveMonth(invalidDate, 1);
    // ...
    equal(this.input.val(), "31-03-2011", "data is reset");
});
...
test('parseDate - outputs correct value', function(){
    ...
    equal(parsedDate.getUTCDate(), "13", "data is correct");
