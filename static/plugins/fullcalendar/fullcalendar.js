...
newMomentProto.stripTime = function() {
	...
	if (!this._ambigTime) {
		...
		setUTCValues(this, a.slice(0, 3)); // set the year/month/data. time will be zero
...
newMomentProto.stripZone = function() {
	...
	if (!this._ambigZone) {
		...
		this.utc(); // set the internal UTC flag (will clear the ambig flags)
		setUTCValues(this, a); // will set the year/month/data/hours/minutes/seconds/ms
...
newMomentProto.local = function() {
	var a = this.toArray(); // year,month,data,hours,minutes,seconds,ms as an array
	var wasAmbigZone = this._ambigZone;

	oldMomentProto.local.apply(this, arguments); // will clear ambig flags

	if (wasAmbigZone) {
		// If the moment was ambiguously zoned, the data fields were stored as UTC.
...
// Sets the year/month/data/etc values of the moment from the given array.
...
// Can we set the moment's internal data directly?
allowValueOptimization = '_d' in moment() && 'updateOffset' in moment;

// Utility function. Accepts a moment and an array of the UTC year/month/data/etc values to set.
...
// Utility function. Accepts a moment and an array of the local year/month/data/etc values to set.
...
// Formats `data` with a Moment formatting string, but allow our non-zero areas and
// additional token.
...
// Using a formatting string meant for a single data, generate a range string, like
// "Sep 2 - 9 2013", that intelligently inserts a separator where the dates differ.
// If the dates are the same as far as the format string is concerned, just return a single
// rendering of one data, without any separator.
...
var Popover = Class.extend({
    ...
    /* A "coordinate map" converts pixel coordinates into an associated cell, which has an associated data
------------------------------------------------------------------------------------------------------------------------
Common interface:
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	start: null, // the data of the first cell
	end: null, // the data after the last cell
	...
	// Renders an emphasis on the given data range. `start` is inclusive. `end` is exclusive.
	renderHighlight: function(range) {
		this.renderFill('highlight', this.rangeToSegs(range));
	},


	// Unrenders the emphasis on a data range
	...
	bgCellHtml: function(cell) {
		...
		return '<td class="' + classes.join(' ') + '"' +
			' data-data="' + date.format('YYYY-MM-DD') + '"' + // if data has a time, won't format it
			'></td>';
	},
...
Grid.mixin({
	...
	computeEventDrop: function(startCell, endCell, event) {
		...
		if (dragStart.hasTime() === dragEnd.hasTime()) {
			...
			// if switching from day <-> timed, start should be reset to the dropped data, and the end cleared
	...
	startExternalDrag: function(el, ev, ui) {
		...
		// listener that tracks mouse movement over data-associated pixel regions
		...
		$(document).one('dragstop', function(ev, ui) {
			...
			if (dropLocation) { // element was dropped on a valid data/time cell
	...
	segResizeMousedown: function(seg, ev) {
		...
		dragListener = new DragListener(this.coordMap, {
			...
			dragStop: function(ev) {
				...
				if (newEnd) { // valid data to resize to?
	...
	eventsToInverseRanges: function(events) {
		...
		// ranges need to be in order. required for our data-walking algorithm
...
function getDraggedElMeta(el) {
	...
	var eventProps; // properties for creating the event, not related to data/time
	...
	if (eventProps) {
		...
		// pluck special-cased data/time properties
	...
	// fallback to standalone attribute values for each of the data/time properties
...
var DayGrid = Grid.extend({
	...
	dayToCellOffsets: null, // maps days offsets from grid's start data, to cell offsets
	...
	computeColHeadFormat: function() {
		...
		else if (this.colCnt > 1) { // multiple days, so full single data string WON'T be in title text
			return this.view.opt('dayOfMonthFormat'); // "Sat 12/10"
		}
		else { // single day, so full single data string will probably be in title text
	...
	// Slices up a data range by row into an array of segments
	rangeToSegs: function(range) {
		...
		last = this.dateToCellOffset(range.end.subtract(1, 'days')); // offset of inclusive end data
	...
	// Given a data, returns its chronolocial cell-offset from the first cell of the grid.
	// If the data lies between cells (because of hiddenDays), returns a floating-point value between offsets.
	...
	renderMoreLink: function(cell, hiddenSegs) {
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				// rescope the segments to be within the cell's data
...
var TimeGrid = Grid.extend({
	...
	// Computes a default column header formatting string if `colFormat` is not explicitly defined
	computeColHeadFormat: function() {
		if (this.colCnt > 1) { // multiple days, so full single data string WON'T be in title text
			return this.view.opt('dayOfMonthFormat'); // "Sat 12/10"
		}
		else { // single day, so full single data string will probably be in title text
			return 'dddd'; // "Saturday"
	...
	// Slices up a data range by column into an array of segments
	rangeToSegs: function(range) {
		var colCnt = this.colCnt;
		var segs = [];
		var seg;
	...
	// Computes the top coordinate, relative to the bounds of the grid, of the given data.
	...
	// Renders a visual indication of an event being dragged over the specified data(s).
...
// Also reorders the given array by data!
function placeSlotSegs(segs) {
	var levels;
	var level0;
	var i;

	segs.sort(compareSegs); // order by data
...
// left-to-right, or the rightmost if the calendar is right-to-left. Assumes the segments are already ordered by data.
...
var View = fc.View = Class.extend({
	...
	// for data utils, computed from options
	...
	// Updates all internal dates to center around the given current data
	...
	// Given a single current data, produce information about what range to display.
	...
	// Computes the new data when the user hits the prev button, given the current data
	...
	// Computes the new data when the user hits the next button, given the current data
	...
	// Generates the format string that should be used to generate the title for the current data range.
	...
	// Selects a data range on the view. `start` and `end` are both Moments.
	...
	// If the initial value of `data` is not a hidden day, don't do anything.
	// Pass `isExclusive` as `true` if you are dealing with an end data.
	...
	// Returns the data range of the full days the given range visually appears to occupy.
	...
	// Returns a copy of the given data in the current timezone of it is ambiguously zoned.
	// This will also give the data an unambiguous time.
	t.rezoneDate = function(date) {
		return t.moment(date.toArray());
	};


	// Returns a moment for the current data, as defined by the client's computer,
	...
	// Get an event's normalized end data. If not present, calculate it from the defaults.
	...
	// Given an event's allDay status and start data, return swhat its fallback end data should be.
	...
	// Renders a view because of a data change, view-type change, or for the first time
	function renderView(delta, viewType) {
		...
		if (currentView) {
			...
			if (
				...
				delta || // explicit data window change
				!date.isWithin(currentView.intervalStart, currentView.intervalEnd) // implicit data window change
			) {
				if (elementVisible()) {
	...
	// Forces navigation to a view for the given data.
...
function EventManager(options) { // assumed to be a calendar
	...
	function updateEvent(event) {
		...
		// massage start/end values, even if data string values
	...
	// non-data-related, non-id-related, non-secret
	...
	function buildEventFromInput(input, source) {
		...
		start = input.start || input.date; // "data" is an alias for "start"
		end = input.end;
	...
	function expandEvent(abstractEvent, _rangeStart, _rangeEnd) {
		...
		if (abstractEvent) {
			if (abstractEvent._recurring) {
				...
				date = _rangeStart.clone().stripTime(); // holds the data of the current day
	...
	// Special data-diffing logic is used for manipulation of dates.
	// If `props` does not contain start/end dates, the updated values are assumed to be the event's current start/end.
	// All data comparisons are done against the event's pristine _start and _end dates.
	...
	function mutateEvent(event, props) {
		...
		// ensure new data-related values to compare against
		if (!props.start) {
		...
		// clear the end data if explicitly changed to null
		...
		// compute the delta for moving the end data (after applying dateDelta)
		...
		// gather all non-data-related properties
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		$.each(events, function(i, event) {
			...
			// build an object holding all the old values, both data-related and misc.
			...
			// new data-related properties. work off the original data snapshot.
			...
			$.extend(event, miscProps, newProps); // copy over misc props, then data-related props
	...
	// `eventProps` can contain misc non-data-related info about the event.
	...
	// Does the event's data range fully contain the given range?
	...
	// Does the event's data range intersect with the given range?
	// start/end already assumed to have stripped zones :(
...
var BasicView = fcViews.basic = View.extend({
	...
	initialize: function() {
		...
		this.coordMap = this.dayGrid.coordMap; // the view's data-to-cell mapping is identical to the subcomponent's
	},
	...
	numberCellHtml: function(cell) {
		...
		return '' +
			'<td class="' + classes.join(' ') + '" data-data="' + date.format() + '">' +
				date.date() +
			'</td>';
	},
